...
### 2026-06-09: Correctness-oracle investigation + Smuggle (§14) mechanic

**Oracle investigation (the gate for scaling the bespoke tail) → [docs/ORACLE_INVESTIGATION.md](docs/ORACLE_INVESTIGATION.md).** Researched reference engines. **Karabast/`forceteki` is MIT-licensed TypeScript with per-card Jasmine behavior specs** (`player.setDeck()` / `setSpaceArenaUnits()` / `toBeInLocation()`). Key conclusion: **its value to us is its TEST CORPUS, not its engine** — reframes "oracle" from live differential testing (costly: prompt/server model mismatch with our pure `step`/chooser engine) to **adapting their per-card golden tests** into our `scenarios.ts` harness (bounded, per-card, LLM-assistable, independently sourced). Recommended sequence: (1) cheap now — fuzzer state-invariants + LLM-judge; (2) the real oracle — port forceteki specs (PoC on ~10 known cards first); (3) only then LLM-generate the bespoke tail, gated by `validate.ts` + the oracle. Do NOT build live differential testing first. Metric: "% verified correct," not "% translated."

**Smuggle [Y] (§14) — implemented (deferred keyword, now functional).** New `SMUGGLE` player action: play a card from your OWN resource zone for its bracket cost instead of from hand, replacing it with the top of your deck. Reuses the `playFromResourceZone` core (built for Plot), parametrized with a `cost` override + `includeSelf` (§14e — a Smuggle card can be exhausted to help pay its own cost). Units enter play exhausted (§14f); the played card is replaced by the top of deck, exhausted (§14g); empty deck → no replacement. Bracket cost parsed from "Smuggle [N resources …]" → the keyword's value (translate); **aspect penalties + additional bracket costs are NOT modeled — consistent with the engine ignoring aspect penalties for normal play too** (verified: no penalty logic exists anywhere). Surface: `actions.ts` (new kind), `translate.ts` (bracket value + events now carry keywords), `cost.ts` (`smuggleCostOf`), `legal.ts` (offer + `describeAction`), `reducer.ts` (`applySmuggle` + dispatch), `keywords/smuggle.ts` (marker). Fixture W8_067 + engine scenario (smuggle → arena exhausted, 2 spent, top-of-deck replacement). Real cards verified to extract the bracket cost ("Smuggle [9 …]" → 9). **fuzz 500 clean** — the new action entered the random action set without issue (the critical net for a new action type). Engine 185 → **186** / translate **108** / validate 16; tsc clean. Coverage metric unchanged (Smuggle is a keyword, counted separately from ability clauses) — the win is the mechanic working in play.

### 2026-06-09: Scaling — matcher-reach batch + a key recalibration (the "giant" buckets are inflated by compounds)

**Worked the highest-count matcher-reach buckets (exhaust/ready [76], cost-change [54]) — and learned they're mostly illusory.** The mechanic tally bucketed any residual clause *containing* the keyword, but exhaust/ready/cost overwhelmingly appear *inside* bespoke compounds (disclose payoffs, "opponent discards", `Action [exhaust]: play a unit … it costs N less`, modals) that fail for *other* reasons. The real standalone delta from clean templates was small:
- **exhaust/ready:** generalized the exhaust template (enemy|friendly, non-leader, ground|space, "with N or less remaining HP"), added "Exhaust attached unit." and an effect-level "If this unit is upgraded, \<effect\>" (reuses the existing `self_upgraded` predicate — the constant form already existed). "Exhaust a unit." with no qualifier now resolves to *any* unit (chooser's pick) rather than defaulting to enemy. **+8 cards.**
- **cost-change:** the only clean part was a near-miss — cards saying "costs N **resource** less to play …" that the four `parseCostClause` regexes rejected (they expected "costs N less"); made "resource(s)" optional. **+3 cards.** The rest of that bucket is `Action [exhaust]: play-from-hand-at-a-discount` compounds — bespoke.
- Corpus **777 → 788 (+11)**. Engine 185 / translate 106 → **108** / validate 16; tsc clean, fuzz 300 clean. +2 matcher scenarios.
- **The finding that matters for strategy:** the matcher-reach "giants" overstated by ~9× (76 → +8 real; 54 → +3 real). Combined with the new-primitive batches yielding tens each, **regex templating has hit clear diminishing returns.** ~750 fully-residual cards remain and they are dominated by bespoke compounds. The next real leverage is NOT more templates — it's the bespoke tier: a **correctness oracle** (differential vs a reference engine / golden per-card tests) + **LLM-generated AST specs**, plus the few remaining *chunky* true-primitives if we want more mechanics breadth (Smuggle, Piloting, force-triggers).

### 2026-06-08: Scaling — mechanic-level residual tally + first two reusable predicate leaves (controls_named, has_initiative)

**Strategy shift toward the full pool (Christian's call): build reusable *primitives* (composable building blocks), not bespoke per-card regex.** Ran a corpus-wide *mechanic-level* residual tally (bucket every unmatched clause by the primitive it needs, count distinct cards): **1,535 residual cards, 783 (51%) entirely bespoke compounds** (the LLM/authoring tail), the rest clustering into ~30 mechanic buckets. Top buckets split into *true missing primitives* (Smuggle 34, force-triggers 25, Piloting 16, defeat-upgrade 17, initiative 13, protection 10, named-control 10, Support 8) vs *matcher-reach* (exhaust/ready 76, cost-change 54, search 49, give-shield 46, disclose 31 — the primitive exists, the phrasing doesn't parse).
- **First two building blocks (the compositional thesis, proven):** new predicate leaves `controls_named` ("If you control Poe Dameron (as a unit, upgrade, or leader), …" — scans units + attached upgrades + leaders by printed name) and `has_initiative` ("While you have the initiative, …"). Each is ~5 lines in `predicates.ts` + a parameterized matcher rule; both compose with the *existing* `if` / constant-`while` / buff / damage / create_token primitives. Black One, Senator's Aide, Jedi Knight, Taylander Shuttle all → `coverage: full` purely by composition.
- Corpus fully-playable **748 → 765 (+17 cards)** from two tiny leaves; and every future character-control / initiative card now matches for free. Fixtures W8_065/066 + 2 engine scenarios (initiative gating; controls_named as unit/leader, opponent-doesn't-count) + 2 matcher scenarios. Engine 181 → **183** / translate 103 → **105** / validate 16; tsc clean, fuzz 300 clean.
- **More primitives (same session): `defeat_upgrade` + Force-conditional matcher-reach.** (a) New `defeat_upgrade` effect — upgrades aren't units so the generic `defeat` couldn't target them; this resolves up to `count` upgrades on the scoped controller's units (filter e.g. non-unique), or the source upgrade (`self` — "Defeat this upgrade."), detaching each to its owner's discard via the same routing `state_based` uses on host defeat. Matcher: "Defeat [up to N] [enemy|friendly] [non-unique] upgrade(s)." / "Defeat this upgrade." (~17 cards). (b) **Force-conditional was pure matcher-reach** — `player_has_force_token` already existed, so "While the Force is with you, this unit gains \<Keyword\>/gets +N/+N" and the friendly-aura form needed *only* a matcher rule, no engine code (Plo Koon, Jedi Sentinel, The Son). Corpus **765 → 777**; units full 270 → 280. Fixtures reused (W8_058 upgrade); +2 engine + 2 matcher scenarios. Engine 183 → **185** / translate 105 → **107** / validate 16; tsc clean, fuzz 300 clean. **Net for the scaling push: 748 → 777 fully playable (+29) from ~3 small primitives + matcher-reach — the compositional thesis holding.**
- **Roadmap recorded** (PROGRESS): work the true-missing-primitive buckets by leverage; treat the matcher-reach buckets as parameterized-template work; the 783 bespoke cards are the authored/LLM tier — which needs a correctness oracle (differential vs a reference engine, or golden per-card tests) stood up *before* mass-generation.

### 2026-06-07: Playtest feedback — Captain Rex deploy bug fixed + card-hover preview + SaveDeckDialog restyle

Three items from a live playtest:
- **Captain Rex created no Clone token on deploy — FIXED (matcher gap).** His leader `deploy_box` is "When Deployed: Create a Clone Trooper token." + "Each other friendly Trooper unit gets +0/+1." The matcher had **no `When Deployed:` trigger prefix** (→ `coverage: none`, so the deploy did nothing) and no trait-filtered "Each [other] friendly `<Trait>` unit gets +N/+N" aura. Added both: the `When Deployed:` prefix → `event.leader_deployed` (the engine already emits LEADER_DEPLOYED with `leaderIid` = the deployed unit's iid, so `card: 'self'` matches; verified end-to-end — deploy now creates the token), and the trait aura (with `exclude: {self}` for "other"). Captain Rex deploy_box → `coverage: full`. Fixture W8_064 + engine scenario (deploy → token + ally +0/+1) + matcher coverage. Engine 180 → **181**.
- **Card-hover preview in /playtest (new).** Hand chips and board units now show a floating preview panel on hover (name, subtitle, cost, type/arena, aspects, live stats, traits, and printed oracle text). Needed a tiny data thread: `CardSpecBase.text` (carried through `translateCard` from `card.text`; inert to the engine, purely for UI). New `app/playtest/CardHover.tsx` (cursor-anchored portal panel, Twin Suns theme). Verified in-browser: hovering hand cards renders the panel with correct data; `spec.text` confirmed populated for real (translated) cards so their oracle text displays (the built-in fixture deck shows none because its cards are hand-authored AST specs without printed text).
- **SaveDeckDialog restyled to the Twin Suns theme.** It was still generic `gray-900/purple`; now uses the `ts-*` design system (parchment/amber, `ts-display` heading, `ts-eyebrow`, `ts-input`, `ts-btn`/`ts-btn-primary`, amber summary values) to match the rest of the app.
- tsc clean; engine 181 / translate 103 / validate 16. No engine-core behavior change (matcher + UI only).

### 2026-06-07: On-Attack-before-combat ordering — now COMPLETE (nested ability-attacks + Ambush too)

**Closed the remaining gap from the combat-ordering fix (Christian confirmed the §7.x rule applies to ability-driven attacks identically).** The prior pass fixed the player ATTACK action; nested ability-attacks (`attack` effect — "this unit attacks again" / "attack with a unit") and Ambush still ran On-Attack triggers AFTER combat because they go through the combined `resolveAttack`, which couldn't reach the settle machinery without an import cycle.
- **Fix:** the reducer injects a `DeclaredAttackResolver` into `attack.ts` (`setDeclaredAttackResolver`, called at reducer module load — a setter breaks the cycle). The combined `resolveAttack` now does declare → resolve declared triggers → recompute power → combat, returning ONLY the combat events (the declared + On-Attack events are fully settled inside the resolver, so the caller's own settle can't re-fire On-Attack). Fixes nested attacks AND Ambush uniformly through one injection point. Falls back to the pre-fix combined behavior if no resolver is injected (hand-built contexts).
- **Re-entrancy safety (the crux):** a nested attack often resolves DURING a trigger drain, so the resolver runs a settle re-entrantly. It isolates the outer pending-trigger queue (save → set to empty → settle just the declared events → restore), so it resolves only this attack's On-Attack/when-attacked triggers and never drains unrelated sibling triggers. **fuzz 2000 clean** — the regression net for the re-entrant path.
- Verified: a Condemned 9-power unit making a nested `attack` effect deals 3 when the defender discloses (the −6 applies before combat), proving the ability-attack path now matches the player path. New engine scenario. Engine 179 → **180**; all prior scenarios unchanged. **On-Attack now sequences before combat damage on every attack path (player, nested ability, Ambush).**

### 2026-06-07: Whole-card focus #6 — Condemn complete + CORE COMBAT-ORDERING FIX (On-Attack abilities now resolve before combat damage, §7.x)

**Condemn surfaced a real, general engine bug — and we fixed it properly (Christian's call).** Deck "Test for Claude" 48/52 → **49/52 (94%)**.
- **The bug:** `resolveAttack` dealt combat damage inline using a power snapshot, but On-Attack *triggered* abilities settled AFTER the attack returned. Per SWU §7.x the attack sequence is declare → resolve "On Attack" / "when attacked" abilities → THEN deal combat damage. So any combat-modifying On-Attack ability (Condemn's −6/−0, an On-Attack buff/debuff, "On Attack: deal damage to the defender" that should kill it pre-combat) landed too late. Proven empirically (Condemn's −6 left a 9-power attack dealing 9, not 3).
- **The fix:** split `resolveAttack` into `declareAttack` (exhaust + ATTACK_DECLARED + keyword onAttack hooks + Saboteur) and `resolveCombat` (recompute attacker power from CURRENT state + deal damage). The reducer's player-ATTACK path now interleaves a `settle` between them, so On-Attack triggered abilities resolve and the combat power reflects them. `resolveCombat` also gracefully fizzles (no combat damage) if the attacker or a unit defender left play during the On-Attack window (§7.x) — previously it threw. Nested ability-attacks + Ambush keep the combined `resolveAttack` (they can't settle mid-attack without an import cycle; a documented smaller gap). **All 176 prior scenarios pass unchanged (the split is behavior-preserving when called back-to-back), and fuzz 1000 is clean** — the regression net for the most-used code path.
- **Condemn itself** ("While attached unit is attacking, it gains: 'On Attack: The defending player may disclose Vigilance/Villainy. If they do, this unit gets −6/−0 for this attack' and loses all other abilities"): new `ConstantAbility.while_attacking` flag; the granted On-Attack body parses to `if_did(optional(disclose by the defending player, Vig/Villainy filter), give −6/−0 end_of_attack to the host)`; `lose_all_abilities` (previously an inert Modifier field) is now honored in trigger collection — a `while_attacking` + `lose_all_abilities` host has its OWN abilities suppressed on its attack events (only the granted Condemn ability resolves). The stat aggregator skips `while_attacking` constants (their modifier carries no stats). Matcher: new `parseWhileAttackingGrant` + a dedicated disclose-debuff effect parse (handles the concatenated "VigilanceVillainy" aspect run + en-dash deltas). Verified end-to-end: disclose → 9-power attack deals 3; decline → 9; Field Commander's own On-Attack draw is suppressed under Condemn (control without Condemn draws). Fixture W8_063 + 3 engine scenarios + 1 matcher scenario.
- Engine 176 → **179** / translate 102 → **103** / validate 16; tsc clean, fuzz 1000 clean. **Remaining deck gaps: Corvus (Piloting dual-play-type path), Overwhelming Barrage (deferred), Incinerator Trooper (false positive, works in-game)** → effective ≈51/52.

### 2026-06-06: Whole-card focus #4–5 — Regional Governor + Chancellor Palpatine complete (deck → 92%)

**Two more whole cards finished in one pass; deck "Test for Claude" 46/52 → 48/52 (92%), crossing 90%.** Both translate `coverage: full`.
- **Regional Governor — "When Played: Name a card. While this unit is in play, opponents can't play the named card."** New `name_card` effect: the source unit's controller names one of the cards an opponent could play (distinct names across the opponent's hand/deck/discard — bounded + deterministic-default), recorded as `CardInstance.namedCard`. The continuous play-restriction is a new shared `isPlayNameBlocked(state, pid, cardName)` (zones.ts, matches by NAME so all copies/variants are blocked), enforced in BOTH `legal.ts` (PLAY_CARD generation skips blocked cards) and `reducer.applyPlayCard` (throws) — so legal/reducer agree and the fuzzer can't pick a now-illegal play. The restriction is positional/gated-on-in-play: it ends when the unit leaves play, and only opponents of the controller are restricted (control-transfer flips it correctly). 3 engine scenarios (name+block via legal+reducer; gated on in-play; opponents-only) + 1 matcher scenario.
- **Chancellor Palpatine — "When Played: If you control a leader unit, create 2 Spy tokens and give those tokens Sentinel for this phase."** Three pieces: (a) wired the `card_is_leader_unit` predicate leaf into `evalCardPredicate` (a deployed leader-unit = an in-arena instance whose cardId is a leader spec), consumed via the existing `controller_controls` gate; (b) the matcher's control-conditional now maps "a leader unit" → `card_is_leader_unit` (was mis-reading "leader" as a trait); (c) **the "give those tokens X" created-token-reference problem** — solved by bundling the grant into `create_token` (new optional `CreateTokenEffect.grant: Modifier`), so `applyCreateToken` registers a per-token lasting effect on exactly the just-created tokens (by iid from the TOKEN_CREATED events) with the modifier's duration. No cross-effect "those tokens" binding needed. Verified: with a leader unit → 2 Spy tokens with effective Sentinel this phase that expires at end of phase; without → nothing. 3 engine scenarios + 1 matcher scenario.
- Engine 170 → **176** / translate 100 → **102** / validate 16; tsc clean, fuzz 400 clean. **Remaining deck gaps (2 real + 1 deferred + 1 false positive):** Condemn (while-attacking grant + the inert `lose_all_abilities` field + the `disclose` runtime), Corvus (Piloting dual-play-type path), Overwhelming Barrage (power-divided — deferred), Incinerator Trooper (false positive, works in-game).

### 2026-06-06: Whole-card focus #3 — Death Star Plans complete (upgrade control-transfer + per-round discount)

**Third whole card finished — two new reusable engine primitives, both halves of the card.** Deck "Test for Claude" 45/52 → **46/52 (88%)**; the real card now translates `coverage: full`.
- **Clause 1 — upgrade control-transfer.** New `transfer_upgrade` effect: "When attached unit is attacked: The attacking player takes control of this upgrade and attaches it to a unit they control." The trigger lives on the UPGRADE (not a granted host ability) and uses a new `defender: 'host'` trigger-predicate (`predicates.ts` resolves the upgrade's host via `findHostOfUpgrade` and matches it against `event.defenderIid`). The effect detaches the source upgrade from its host and re-attaches it to a unit chosen by the *attacking player of the triggering attack* (`ctx.triggerEvent.attackerIid` → controller), recording the original controller as the upgrade's `owner` (§8.28.2 discard routing). Note: the existing `attachUpgrade` primitive can't move an already-attached upgrade (`findCard` reports attached upgrades at index −1), so the effect does its own detach+reattach via `findUpgrade` + `mapInstance`.
- **Clause 2 — passive per-round discount.** New `round_discount` ability ("The first unit you play each round costs N less"), granted to the host via the existing `grant.abilities` path. Key simplification: **each SWU round has exactly one action phase and cards are only played then**, so "first … each round" ≡ "first … this action phase." Rather than a board scan in `effectiveCost`, the discount is *armed* at `startActionPhase`: new exported `roundDiscountsToArm(state, reg)` (triggers.ts) scans in-play units for intrinsic + granted `round_discount` abilities and pushes a phase-scoped `PendingDiscount` per controller — reusing General's Blade's existing consume-on-first-matching-play + end-of-phase-clear machinery wholesale. No change to `effectiveCost` or the consume path.
- Verified: clause 1 moves the upgrade onto the attacker's unit (1/9 trade so both survive, owner=defender-side recorded); clause 2 arms exactly one unit-typed discount for the upgrade-controller, leaves events unaffected, and an end-to-end round transition arms it at round start and consumes it on the first unit. Fixture W8_060 + 3 engine scenarios + 1 matcher scenario. Engine 167 → **170** / translate 99 → **100** / validate 16; tsc clean, fuzz 400 clean.

### 2026-06-06: Whole-card focus #2 — Nimble Prowess complete (host-arena scoping)

**Second whole card finished.** Nimble Prowess ("When Played: You may exhaust a unit in attached unit's arena") needed one new piece: a **`host_arena` zone filter** that resolves to whichever arena the upgrade's host sits in. Added to `ZoneFilter`; `resolveSelector` special-cases it (via a new `upgradeHostArena` helper) before the normal zone expansion, so the target pool is exactly the host's arena. The upgrade's When-Played fires correctly now (thanks to the findCard fix from #1). Verified: with a ground host, a ground enemy is a candidate but a space enemy is not. Nimble Prowess full → deck **85% → 87%** (45/52). Fixture W8_059 + engine scenario (ground-only candidate) + matcher scenario. Engine 166 → **167** / translate 98 → **99** / validate 16; tsc clean, fuzz 400 clean.

### 2026-06-06: Whole-card focus — The Darksaber complete (+ a real engine bug fixed)

**Switched strategy (Christian's call) from single-clause templates to finishing WHOLE cards so they're actually playable. First target: The Darksaber — and it immediately surfaced a genuine engine bug.**
- Built the two missing pieces for clause 2 ("When Played: If there are 4 or more different keywords among friendly units, ready attached unit"): a `controller_distinct_keywords: Range` predicate (counts distinct keyword names across the controller's units + their upgrades) and a "ready attached unit" → `ready` of `attached_to_self`. Clause 1 ("gains Sentinel") was already done.
- **Engine bug found + fixed (the payoff of doing whole cards): `findCard` didn't search attached upgrades**, so an upgrade's OWN triggered ability (sourceIid = the upgrade) couldn't locate its source — and `applyIf` defaults `cond = true` when the source isn't found. So **every upgrade-own `if`-conditional was silently always-true** (The Darksaber readied the host regardless of keyword count). Fixed `findCard` to also search attachments (reporting the upgrade at its host's arena/controller). Verified: 4 distinct keywords → host readied; 2 → not; Sentinel granted either way. Full suite (166) + **fuzz 500 clean** (findCard is used everywhere — the regression net).
- The Darksaber now full → deck "Test for Claude" 83% → **85%** (44/52). Fixture W8_058 + engine scenario (the whole card) + matcher scenario. Engine 165 → **166** / translate 97 → **98** / validate 16; tsc clean. **Lesson: finishing whole cards finds real bugs that clause-templating hides.**

### 2026-06-06: "If a unit left play this phase" — tracker + predicate (mechanism built; 0 cards fully unlocked, honestly)

**Built the left-play-this-phase mechanism (Christian's call, knowing it's low-leverage).** New optional `GameState.leftPlayThisPhase: PlayerId[]` — the controllers of units that left play (defeated / bounced) during the current action phase. Maintained in `processDefeat` (state_based) and `applyReturnToHand` (bounce); reset at action-phase start. New predicate leaf `unit_left_play_this_phase: 'friendly' | 'any'` (reads the tracker against the evaluated card's controller), consumed by an `if`-on-left-play matcher template. Verified end-to-end: a defeat records the controller; the condition correctly gates a friendly-only effect (friendly → fires, enemy-only → doesn't) and an any-unit effect; tracker resets each phase; fuzz 500 clean (it's maintained on the `processDefeat` hot path).

**Result is honest: the predicate works, but 0 of the 4 cards become fully playable** — each pairs the left-play condition with a *different* unmodeled clause: Let's Talk (in-effect cost reduction + "each friendly unit captures…"), Disturbance in the Force (gain-force + give-shield compound), Yoda (draw + top/bottom-deck manipulation, as an Action), Chancellor Palpatine (a separate "Each token unit you create enters play ready" clause). The condition now parses; the surrounding compounds don't. This confirms the matcher-coverage vein has thinned to bespoke per-card compounds — the new predicate is reusable and will pay off as those other primitives land, but the immediate card payoff is nil. Engine 164 → **165** / translate 95 → **97** / validate 16; tsc clean, fuzz 500 clean. Fixture W8_057 + engine + 2 matcher scenarios.

### 2026-06-03: Bounty-granting upgrades — composing three existing mechanisms

**"Attached unit gains: 'Bounty — `<effect>`'" — one matcher line, no engine change.** `parseGrantAbilityClause` now tries `parseBountyClause` per quoted segment (after the triggered-prefix parse), so a granted Bounty clause resolves. This composes three things already built: the **grant.abilities** host-attribution path, the **`controlled_by: 'opponent'`** Bounty resolution (the *defeater* collects it), and the **UPGRADE_DETACHED** recovery that lets a granted When-Defeated fire on the host's own death. Verified end-to-end: a host with Death Mark ("Bounty — Draw 2 cards") is defeated → the **opponent** draws 2, the owner draws 0. 3/9 of the corpus cards full (Death Mark/Public Enemy/Rich Reward; the rest have ramp/search/conditional inner effects — "put the top card into play as a resource", "search the top N…", "if this unit is unique, … instead"). Fixture W8_056 + engine scenario (opponent collects, owner doesn't) + matcher scenario. Engine 163 → **164** / translate 94 → **95** / validate 16; tsc clean, fuzz 500 clean.

### 2026-06-03: Credit tokens — a real economy mechanic (create + spend), made functional

**The `creditTokens` field existed but was inert (nothing created or spent them). Wired up both halves so it's not fake coverage.** A Credit token is a one-shot resource: spend it like a resource to pay a play cost, removed when spent.
- New **`create_credit`** effect ("Create N Credit token(s)") → adds tokens to the controller's `creditTokens`. Matcher recognizes it *before* the unit-token template (Credits aren't unit tokens).
- **Spend side:** `applyPlayCard` now pays a card's cost from ready resources first, then tops up from Credit tokens (removing the ones spent); affordability (legal.ts) counts Credits toward what a card costs.
- **Fuzz caught a real legal/reducer mismatch (the value of the net):** my first cut added Credits to the shared `readyResourceCount`, which legal.ts also uses to offer *action abilities* with resource costs — but the action-ability reducer doesn't spend Credits, so it threw "Insufficient resources" (6/1000 games). Fix: Credits count only toward **PLAY_CARD** affordability (a separate `playAffordCount`); action-ability/deploy credit-spending is explicitly deferred. **1000-game fuzz clean** after the fix.
- Verified end-to-end: create 2 Credits, then a cost-3 card is affordable with 1 resource + 2 Credits, and both Credits are consumed. 6/23 of the corpus "Create a Credit token" cards now full (the rest gated on still-unmodeled riders / arena-control conditions). Fixture W8_055 + engine scenario (create + spend + removal) + matcher scenario. Engine 162 → **163** / translate 93 → **94** / validate 16; tsc clean, fuzz **1000** clean.

### 2026-06-03: Control-conditional effects — "If you control [another] <X> unit, <effect>"

**The conditional-effect sibling of the "While you control" constant.** "If you control [another] `<X>` unit, `<effect>`." → an `if` effect gated on the `controller_controls` predicate (the same one built for the "While" form). The existing `if` effect already evaluates its condition against the source's controller, so this was a matcher template + reusing the predicate. The condition handles trait/aspect, plus **damaged** (`self_damage ≥ 1`) and **exhausted** (`self_exhausted`); arena ("a ground unit") and compound ("a ground unit AND a space unit") conditions stay residual. A leading "you may" on the inner effect wraps it as optional; the inner effect must itself parse or the whole clause is residual. **9/24** of the corpus shape now full (the rest gated on still-unmodeled inner effects — credit tokens, etc.).

Also widened the **single-target damage** template to "Deal N damage to an [enemy] [ground|space] unit" (arena scope), which unblocks these conditional effects and several other cards. Fixture W8_054 + engine scenario (effect fires only with a matching ally) + 2 matcher scenarios (if-on-control + compound-residual). Engine 161 → **162** / translate 91 → **93** / validate 16; tsc clean, fuzz 500 clean. **Next on the control-conditional family:** the inner-effect gaps (credit tokens — 7 cards — would unlock several of these *and* their own residual theme).

### 2026-06-03: Control-conditional self-buffs — "While you control [another] <X> unit, …"

**Second-largest residual theme after damage: control-conditionals (~35 cards). Did the cleanest, highest-frequency sub-shape — the constant self-buff/grant.** "While you control [another] `<Trait/Aspect>` unit, this unit gets +N/+N / gains `<Keyword> [N]`." **16/16 of that exact shape now full** (Villainy/Official/Aggression/Republic/Fighter/Force/Vehicle/…).

- New **`controller_controls`** predicate leaf — `{ filter?: Predicate; exclude_self?: boolean }` — true iff the controller has an in-arena unit matching `filter`, optionally excluding the unit being evaluated ("**another**"). Generalizes the old `controller_controls_trait` (trait-only, no "another") to aspects, power conditions, and the exclude-self case; evaluated via `evalCardPredicate` so any card-predicate filter works. Verified: a "While you control another Villainy unit" buff is inert alone (its own Villainy doesn't count), active with a second Villainy unit, inert with only a Heroism ally.
- Matcher: one general template (replacing the narrow trait-only one) handles "another" vs "a", trait vs aspect (`aspectOrTrait`), and either a stat buff (+N/+N) or a keyword grant (gated against the keyword set). The old trait-only Bunker Defender matcher scenario was folded into the new ones.
- Fixture W8_053 + engine scenario (gated, excludes self) + 2 matcher scenarios. Engine 160 → **161** / translate 90 → **91** / validate 16; tsc clean, fuzz 500 clean. Remaining control-conditionals: the "**If** you control …, `<effect>`" form (a triggered effect gated on a control predicate — needs an `if`-on-control compound) and "or upgrade"/"unique"/"with N power" condition variants.

### 2026-06-03: Damage-target batch — `chosen_base` + "this unit" / "each base" / arena AOE

**Switched from attack riders to the single largest residual theme (measured corpus-wide): "Deal N damage to …", ~50 residual cards.** Knocked out the highest-frequency unhandled targets in one batch:
- **`chosen_base` selector** (new) — "Deal N damage to a base": the source player chooses any base, with the opponent's offered first so the default/AI chooser picks it (sane aggressive default). 7 cards. `applyDamage` already handled base targets, so this was a selector + matcher addition. Verified: default → opponent's base, scripted → own base.
- **"Deal N damage to this unit"** → `{ self: true }` (3 cards); **"…to your base"** → `self_base`; **"…to each base"** → a sequence over both bases (2); **"…to each [ground|space] unit"** — extended the AOE template with arena scope (2).
- A corpus residual-frequency pass (`matchCard` over all 1.5k deckable cards, tallying 4-word clause prefixes) drove the choice: the damage family was the top theme, ahead of control-conditionals ("while/if you control …", ~35) and the attack riders already in progress.
- New selector `chosen_base` added to the validator's flag set. Fixture W8_052 + engine scenario (default vs chosen base) + matcher scenario (the whole batch). Engine 159 → **160** / translate 89 → **90** / validate 16; tsc clean, fuzz 500 clean.

### 2026-06-03: Attack-rider #2 — defender debuff (Catch Unawares / Jyn Erso)

**Second rider, the clean mirror of `attacker_buff`.** "Attack with a unit. The defender gets -N/-0 for this attack." New `defender_debuff?: Modifier` on the `attack` effect — applied to the CHOSEN defender (units only; never the base) as a lasting effect inside the attack loop, before `resolveAttack` and removed after, so it lowers the defender's return damage for exactly this attack. Verified: a debuffed defender deals reduced return damage and the effect is cleaned up. Matcher: refactored the attack rider parsing to a head + dispatched riders (attacker buff / defender debuff / fall-through-residual), and the defender-debuff regex accepts hyphen-minus, en-dash, and em-dash. Corpus attack-with-a-unit 9/35 → **11/35**. Fixture W8_051 + engine + matcher scenario. Engine 158 → **159** / translate 88 → **89** / validate 16; tsc clean, fuzz 500 clean.

### 2026-06-03: Attack-rider #1 — conditional attacker buff (Snowtrooper Lieutenant)

**First rider on the attack-with-a-unit foundation, chosen by corpus frequency: the attacker-identity conditional buff.** "Attack with a unit. If it's an Imperial/Rebel/Bounty Hunter unit, it gets +N/+0 for this attack." Unlocks **Snowtrooper Lieutenant** (in the deck → 83%) plus the Rebel/Bounty-Hunter siblings; corpus attack-with-a-unit coverage 6/35 → **9/35**.

- New `attacker_buff_if?: Predicate` on the `attack` effect — the `attacker_buff` is applied only if the CHOSEN attacker matches (evaluated against the attacker the moment it's resolved). The attack happens regardless; only the buff is gated. Verified: an Imperial attacker hits for +2, a Rebel for +0.
- Matcher: extended the attack template with an optional "If it's a/an `<Trait/Aspect>`, " prefix on the buff clause, parsed via the shared `parseHostCondition` (handles multi-word traits like "Bounty Hunter"). The **"If it's a/an"** shape naturally excludes the *defender*-condition form ("If it's attacking a unit") — that stays residual, as it's a runtime defender check, not an attacker-identity one.
- Fixture W8_050 + engine scenario (Imperial buffed, non-Imperial not) + 2 matcher scenarios (conditional buff + defender-condition residual). Engine 157 → **158** / translate 86 → **88** / validate 16; tsc clean, fuzz 500 clean. Remaining riders (each an incremental template on this core): defender debuff ("The defender gets -N/-0 for this attack", ~6 cards), per-X attacker buff ("+1/+0 for each card in your hand"), post-attack riders ("After completing this attack, defeat it").

### 2026-06-03: Highest-overlap primitives — "Attack with a unit" + unconditional keyword grants

**Chose the two patterns with the most corpus reach (measured by DB frequency before building): "attack with a unit" appears in ~35 cards, "Attached unit gains `<Keyword>`" in ~16.** Net: **11 more cards fully resolve corpus-wide** (6 attack-with-a-unit + 5 keyword grants), and the new `attacker_buff` machinery is the foundation for the many remaining attack riders.

- **Unconditional keyword grant (matcher-only, machinery already existed).** Extended `parseAttachedKeywordGrant` to also parse "Attached unit gains `<Keyword>` [N]." (e.g. The Darksaber/Sentinel, Devotion/Restore 2, Grievous's Wheel Bike/Overwhelm) alongside the existing conditional form — both → a constant grant to the host. The keyword is gated against the known-keyword set, so **trait grants** ("gains the Mandalorian trait") correctly stay residual (the engine has no trait-grant modifier yet — separate, heavier work). 5/7 of the simple cases now full.
- **"Attack with a unit. It gets +N/+0 [and gains K] for this attack."** The `attack` effect already supported a `attacker` selector; added an **`attacker_buff: Modifier`** applied to the chosen attacker as a lasting effect FOR THIS ATTACK — added before the attack loop, **removed after** (verified: the attacker's power returns to base and no lasting effect leaks). Matcher: a chosen **ready** friendly attacker (`self_exhausted: false`; "you can only attack with a ready unit"), an optional cost filter ("that costs N or less"), and the optional buff (power/health + optional granted keyword, gated against the keyword set). Variants with a *conditional* buff ("If it's attacking a unit…") or a post-attack rider ("After completing this attack, defeat it") stay residual (honest) — 6/35 full now, but the core + buff covers the simple shape and unblocks the rest.
- Fixture W8_049 + engine scenario (buff applies during combat, cleaned up after) + 4 matcher scenarios (keyword grant + trait-residual + attack buff + optional/conditional). Engine 156 → **157** / translate 82 → **86** / validate 16; tsc clean, **fuzz 500 clean** (the buff is on the combat path).

### 2026-06-03: Two contained wins — Death Trooper (two-target damage) + Doctor Pershing (damage-as-cost)

**Both shipped in one pass; deck coverage 77% → 81% (42/52, crossed 80%).**

- **Death Trooper — "Deal 2 damage to a friendly ground unit and 2 damage to an enemy ground unit."** Matcher-only: a two-sentence compound → `sequence` of two chosen-target `damage` effects with ground/space-scoped friendly + enemy selectors (mirrors the existing shield "to a friendly and to an enemy" template).
- **Doctor Pershing — "Action [Exhaust, deal 1 damage to a friendly unit]: Draw a card."** New `damage` field on `ActionAbilityCost` (`{ amount, target }`), paid in `applyActionAbility` through the normal damage layer (shields/replacements apply) after the other costs. `parseActionCost` now recognizes "deal N damage to a friendly unit" as a cost component (previously left the whole action residual — the stale "uninterpretable cost → residual" scenario was converted to a positive test, plus a new residual test using a genuinely-unmodeled cost). Verified: the action draws a card and deals 1 to the chosen friendly (Pershing himself), and exhausts.
- Fixtures W8_047 (Death Trooper) / W8_048 (Doctor Pershing) + 2 engine scenarios + 2 matcher scenarios. Engine 154 → **156** / translate 80 → **82** / validate 16; tsc clean, fuzz 400 clean.

### 2026-06-03: Search-and-play-multiple — Darth Vader (Commanding the First Legion)

**`search_play` effect — search the top N, play any number of matching units under a combined-cost cap, for free.** Unlocks **Darth Vader — Commanding the First Legion** ("When Played: Search the top 10 cards of your deck for any number of [Villainy] units with combined cost 3 or less and play each of them for free."); deck coverage 75% → 77%.

- **Refactor first:** extracted the unit-enter-play sequence (move into arena → exhausted §3.4.4b → onPlay keyword hooks → Ambush → CARD_PLAYED) from `applyPlayFromDiscard` into a shared `putUnitIntoPlay(state, reg, iid, pid, spec, chooser)`. Both `play_from_discard` and the new `search_play` reuse it; the unit's When-Played fires from the trailing settle (no inline duplication). play_from_discard scenarios still pass (regression net).
- **The multi-select under a cost cap** is a chooser loop (like Exploit's sacrifice loop): repeatedly offer the still-affordable matches + a "Done" option, accumulating combined cost until the cap is hit or the player stops. The default chooser greedily takes affordable matches in deck order. Chosen units are put into play for free; the deck is shuffled afterward (§8.36, deterministic seed = step). Units only.
- **Matcher:** "Search the top N cards … for any number of [Aspect/Trait] units with combined cost M or less and play each of them for free." → `search_play` with `count`, an optional aspect/trait filter (via the shared `aspectOrTrait`), and `max_combined_cost`. The "[Villainy]" icon markup is handled.
- Verified end-to-end: with a cost-1 + cost-2 + cost-8 Villainy unit and a Heroism unit on top, it plays the cost-1 + cost-2 (combined 3, cap hit), skips the over-cap and wrong-aspect ones, charges nothing, and shuffles. Fixture W8_046 + engine scenario + matcher scenario. Engine 153 → **154** / translate 79 → **80** / validate 16; tsc clean, fuzz 400 clean.

### 2026-06-03: Two contained primitives — Captain Enoch (discard-count buff) + Agent Kallus (unique-defeat trigger)

**Both shipped in one pass; deck coverage 71% → 75% (39/52, crossed 75%).**

- **Captain Enoch — "This unit gets +1/+0 for each Trooper unit in your discard pile."** New per-X count source `controller_discard_units` with an optional `filter` on `Modifier.per` (restricts which discard units count). `perCount` now takes the whole `per` object + `reg` so it can evaluate the filter against each discard card (units only). Matcher: "for each [Trait] unit in your discard pile" → `per: { count: 'controller_discard_units', filter: { card_trait } }`. Verified: 3 printed + 2 Trooper units in discard (a non-Trooper excluded) = 5 power.
- **Agent Kallus — "When another unique unit is defeated: You may draw a card. Use this ability only once each round."** New `card_is_unique` **trigger**-predicate field (reads the event card's spec — the discard copy for a defeat); "another" = `not: { card: 'self' }`, composed via `and`. The once-per-round limit reused the existing limit infra. Two matcher additions: a dedicated "When another unique unit is defeated:" handler (compound `where`, so it sits before the table-driven prefixes) and a **general `extractLimit` suffix parser** ("Use this ability only once each round/phase/game." → `limit`) applied to every triggered clause. Verified: a unique unit's defeat → Kallus's controller draws; a non-unique defeat → no draw.
- Fixtures W8_044 (Enoch) / W8_045 (Kallus) + 2 engine scenarios + 2 matcher scenarios. Engine 151 → **153** / translate 77 → **79** / validate 16; tsc clean, fuzz 400 clean. (Found in testing: most SWU "test" fixtures carry the Trooper trait — used W1_002, a Fighter, as the non-Trooper control.)

### 2026-06-03: Play-from-discard primitive — Palpatine's Return

**`play_from_discard` effect — choose a unit in your discard and play it at a reduced cost.** Unlocks **Palpatine's Return** ("Play a unit from your discard pile. It costs 6 resources less. If it's a Force unit, it costs 8 resources less instead."); deck coverage 69% → 71% (crossed 70%). A new reusable primitive (a "play a card from zone X" sibling of `playFromResourceZone`).

- **Self-contained in `interpret.ts`** to avoid the reducer↔interpret cycle: rather than extract the central `resolveCardIntoPlay` from `reducer.ts`, the effect replicates the small unit-enter-play sequence (move discard→arena, enter exhausted §3.4.4b, CARD_PLAYED, onPlay keyword hooks, Ambush). The unit's triggered **When-Played fires via the trailing settle** on the CARD_PLAYED event (same as a normal play) — no inline duplication. interpret.ts now imports `effectiveCost` (cost.ts), `KEYWORDS`, and `resolveAmbush` (no new cycle — cost.ts imports neither, attack.ts was already imported).
- **Cost model:** `cost_reduction` (flat) with a `cost_reduction_if: { filter, amount }` override when the chosen card matches ("…8 less instead if it's a Force unit" — "Force" is a real trait). Only AFFORDABLE eligible units are offered; if none, the effect is a no-op (verified: a unit whose reduced cost still can't be paid stays in the discard). Reduced cost clamped ≥0.
- **Matcher:** the card text is multi-line, so it's matched at the **whitespace-flattened whole-text level** (like Maximum Firepower), not per-clause. The Force clause is optional in the regex (a plain "Play a unit from your discard pile. It costs N less." also parses).
- Fixtures W8_042 (Palpatine's Return) / W8_043 (a Force unit, cost 8) + 2 engine scenarios (Force unit played free; unaffordable → no-op) + 1 matcher scenario. `emptyState` gained `discardP1/P2`. Engine 149 → **151** / translate 76 → **77** / validate 16; tsc clean, fuzz 400 clean.

### 2026-06-03: Control exchange — Choose Sides (two units swap controllers)

**`exchange_control` effect — a two-way control swap.** Unlocks **Choose Sides** ("Choose a friendly non-leader unit and an enemy non-leader unit. Exchange control of those units."); deck coverage 67% → 69%. Built on the existing `take_control` (§8.28): extracted its single-unit move into a shared `transferUnitControl(state, iid, toController)` helper (positional control — physically move to the new controller's matching arena, keep ready/exhausted + damage + upgrades, record `owner` for the §8.28.2 owner-discard rule, defeat a Leader Unit instead of moving it per §1.6). `take_control` now loops that helper to the source player; `exchange_control` resolves a chosen friendly (→ opponent) and a chosen enemy (→ source) and applies both transfers, resolving both selectors against the original state so picking the enemy isn't perturbed by moving the friendly. Matcher: the two-sentence "Choose … and … . Exchange control of those units." template with non-leader filters. Fixture W8_041 + engine scenario (units swap, damage + owner preserved) + matcher scenario. Engine 148 → **149** / translate 75 → **76** / validate 16; tsc clean, fuzz 400 clean.

This finishes *unit* control-transfer (take_control + exchange). **Death Star Plans still residual:** it transfers an *upgrade* ("the attacking player takes control of this upgrade and attaches it to a unit they control") — a separate mechanism (move an upgrade between hosts + change its controller) — plus the passive per-round discount. Not the unit-control path built here.

### 2026-06-03: Cost-discount primitive + General's Blade (granted On-Attack discount)

**Third slice of "Attached unit gains" — a new dynamic-cost mechanic: "the next unit you play this phase costs N less."** Fully unlocks **General's Blade** ("If attached unit is a Jedi, it gains: 'On Attack: The next unit you play this phase costs 2 resources less.'"); deck coverage 65% → 67% (5 of 7 "Attached unit gains" cards now done).

- **New `discount` effect** — registers a one-shot, phase-scoped play-cost discount on the source player (`PlayerState.discounts: PendingDiscount[]`, keyed by card type so `effectiveCost` stays free of the predicate evaluator). `effectiveCost` subtracts all matching pending discounts (they stack); `reducer.applyPlayCard` consumes the matching ones when the card is actually played; `endActionPhase` clears any unused (phase-scoped). Distinct from the existing `cost` ability (a static modifier to *this* card's own cost) — this is a player-scoped discount on a *future* play.
- **Conditional ability grant** — extended `parseGrantAbilityClause` to also parse "If attached unit is a `<X>`, it gains: '`<ability>`'" (the colon distinguishes the ability-grant from the bare-keyword grant), adding a host `filter` to the grant. **Correctness fix surfaced here:** `grantedAbilitiesForHost` was collecting `grant.abilities` *unconditionally* — now it evaluates the grant's host `filter` (via `evalCardPredicate`) so a Jedi-gated grant is inert on a non-Jedi host. Verified both ways end-to-end.
- Fixtures W8_039 (General's Blade) / W8_040 (a Jedi host) + 2 engine scenarios (Jedi → discount applies, consumed on play; non-Jedi → inert) + 1 matcher scenario. Engine 146 → **148** / translate 74 → **75** / validate 16; tsc clean, fuzz 400 clean.
- **Death Star Plans stays residual (the 7th card):** its cost reduction is a *passive* "the first unit you play each round costs 2 less" (regenerating per round, not a triggered one-shot — needs a constant-ability discount + per-round consumption), AND it has a when-attacked upgrade control-transfer ("the attacking player takes control of this upgrade"). Two more mechanisms; left honest-residual.

### 2026-06-03: Conditional keyword grants ("If attached unit is a Sith, it gains Grit")

**Second slice of the "Attached unit gains" work — host-gated keyword grants.** Covers **Darth Revan's Lightsabers** (Sith → Grit) and **Constructed Lightsaber** (Heroism → Restore 2 / Villainy → Raid 2 / neither → Sentinel); deck coverage 62% → 65% (4 of the 7 "Attached unit gains" cards now done).

This reused the existing modifier-grant flow rather than the trigger path: the grant is a constant `{ grant: { target, modifier: { keyword, keyword_value? } } }`, and the condition lives in a new optional **`filter` on the `attached_to_self` selector** — the host must match or the selector resolves empty, so the keyword is granted only while the host qualifies. `keyword_value` already flowed through `effectiveKeywordValue`, so valued grants (Restore 2 / Raid 2) came for free. Aspect conditions map to `card_aspect` (lowercase icon set), traits to `card_trait` (case-insensitive), and "non-Heroism, non-Villainy" to `{ and: [{ not }, { not }] }`. Matcher: `parseAttachedKeywordGrant` ("If attached unit is a <X>, it gains <Keyword> [N]") with `parseHostCondition` for the trait/aspect/negated-list parsing; the keyword is gated against `KEYWORD_WORDS` so an unknown one stays residual rather than faking coverage. Fixtures W8_037 (Darth Revan's Lightsabers) / W8_038 (a Sith host) + 2 engine scenarios (matching host gains Grit; non-Sith host does NOT) + 2 matcher scenarios. Engine 144 → **146** / translate 72 → **74** / validate 16; tsc clean, fuzz 400 clean. **Remaining 3 of 7:** General's Blade + Death Star Plans (constant/cost *inner* grants — cost reductions), Condemn ("loses all other abilities").

### 2026-06-03: Upgrade-granted abilities ("Attached unit gains: '…'") — host-attributed

**The architectural unlock flagged by the deck audit.** An upgrade that grants a triggered ability to its host ("Attached unit gains: 'On Attack: …'") now works, with the granted ability attributed to the HOST so `self` / `attacker: self` resolve to the host, not the upgrade. Covers **Sith Traditions** (On-Attack give-self-Experience + When-Defeated give-friendly-Experience) and **Vambrace Grappleshot** (On-Attack exhaust the defender); deck coverage 58% → 62%.

Design (the non-obvious parts):
- **AST:** `ConstantAbility.grant` gained an optional `abilities: Ability[]` (and `modifier` became optional — a grant now gives a modifier and/or abilities). On an upgrade the target is the host (`attached_to_self`).
- **Attribution:** `collectTriggers` now appends, for each host unit slot, the abilities granted by its attached upgrades — indexed after the host's own abilities, with `sourceIid = host.iid`. So a granted On-Attack fires when the *host* attacks.
- **Resolution without re-derivation:** a granted ability isn't on the host's spec, so resolving it by `cardAbilities(source)[index]` would miss. The `TriggerInstance` now carries the granted ability inline (`grantedAbility`, pure-data AST), and `drainTriggers` uses it directly. This also makes the trigger resolve even after the source slot is gone.
- **Granted When-Defeated is the tricky case:** when the host is defeated, its upgrades have already detached to discard (processDefeat clears `inst.upgrades`). So `grantedAbilitiesForHost` recovers the granting upgrades for a just-defeated host from that batch's `UPGRADE_DETACHED` events — letting a granted When-Defeated fire on the host's own death. Verified end-to-end (a friendly unit gains Experience after the Sith-Traditions host dies).
- **New selector `trigger_defender`** — the defender of the triggering attack ("Exhaust the defender"), resolved from `ATTACK_DECLARED/ATTACK_ENDED.defenderIid` (gracefully empty when the defender is the base).
- **Matcher:** `parseGrantAbilityClause` parses `Attached unit gains: "<ability>" [and "<ability>"]` (curly or straight quotes) into the grant; each quoted segment must parse as a TRIGGERED ability or the whole clause stays residual (no silent partial grant). Constant/cost inner abilities (cost reductions — Death Star Plans, General's Blade) and conditional keyword grants (Constructed Lightsaber, Darth Revan's Lightsabers) and "loses all other abilities" (Condemn) are NOT yet handled and correctly fall through to residual — the remaining 5 of the 7 "Attached unit gains" deck cards.

Fixtures W8_035 (Sith Traditions) / W8_036 (Vambrace) + 3 engine scenarios + 2 matcher scenarios. Engine 141 → **144** / translate 70 → **72** / validate 16; tsc clean, fuzz 400 clean (the trigger-collection + state-types changes touch every settle, so the fuzz pass is the real net).

### 2026-06-03: Proactive deck audit + Power of the Dark Side (opponent_choose defeat)

**Instead of waiting for Christian to hit each broken card in playtest, ran `deck-coverage` over his real "Test for Claude" deck to enumerate every silent-no-op up front.** Result: 52/52 found, **30/52 playable (58%)**, 22 gaps — each with the primitive it needs. This is the same class of bug as the last two sessions (a `coverage: none` card that plays as a do-nothing event/unit), found proactively rather than one-at-a-time.

- **Power of the Dark Side — FIXED (matcher-only).** "An opponent chooses a unit they control. Defeat that unit." → `defeat` with the existing `opponent_choose` selector (the opponent picks which of *their* units dies). No engine change — selector/AST/validator already supported it; the only gap was the template. Fixture W8_034 + engine scenario (opponent picks which unit, it lands in the owner's discard) + matcher scenario.
- **Audit caveat noted:** the tool keys off the `matchCard` matcher, which doesn't run `translate.parseKeywords` — so the `attacker_combat_first` marker (Incinerator Trooper's "deals combat damage before the defender") shows as residual but actually works in-game. Incinerator is a false positive; real playable count is effectively 31/52.
- **Remaining 21 gaps triaged in PROGRESS** by needed primitive. Biggest single unlock is **upgrade-granted abilities** ("Attached unit gains: …") — one parser covers Sith Traditions, Vambrace Grappleshot, Constructed Lightsaber, Darth Revan's Lightsabers, General's Blade, Condemn, Death Star Plans. The rest are individual bigger primitives (control-exchange, play-from-discard with cost reduction, search-and-play, Spy tokens, "name a card", discard-pile counting). Engine 141 / translate 70 / validate 16; tsc clean, fuzz 200 clean.

### 2026-06-03: Playtest round 2 — Superlaser Technician (When-Defeated → resource)

**Christian confirmed Shatterpoint works; reported Superlaser Technician not becoming a resource on defeat.** Text: "When Defeated: You may put this unit into play as a resource and ready it." It was `coverage: none` — the `play_as_resource` matcher (added for Resupply) only recognized "Put this **event**/card into play as a resource," not "this **unit** … **and ready it**." The engine path already worked: a When-Defeated trigger fires *after* the unit has moved to the discard pile, and `applyPlayAsResource` (which acts on `ctx.sourceIid` wherever it is) moves it from discard to the resource zone. Two changes: (1) widened the matcher regex to `(?:event|card|unit)` with an optional "and ready it" suffix; (2) added a `ready?: boolean` to `PlayAsResourceEffect` so the resource enters **ready** rather than the §2046 exhausted default (Superlaser explicitly readies it). Resupply (no "ready") is unchanged — still enters exhausted. Fixture W8_033 + 2 engine scenarios (accept → ready resource, leaves discard; decline → stays in discard) + 1 matcher scenario. Engine 138 → 140 / translate 68 → 69 / validate 16; tsc clean, fuzz 300 clean. **Overwhelming Barrage** remains deferred (Christian's call) — power-based divided damage + cross-clause "it" binding, not yet built.

### 2026-06-03: Playtest — Shatterpoint reminder bug fixed; Overwhelming Barrage scoped

**Christian's playtest reported two cards "not resolving." One was a real bug, one is an unimplemented mechanic.**

- **Shatterpoint — REAL BUG, FIXED (matcher).** The engine, the async/UI replay protocol, and the modal all handle the card correctly (verified end-to-end: a 3/3 enemy is defeated and moved to discard). The failure was in `parseModalEffect`: it split modal options and stripped bullets but **never applied `stripReminders` per option** (unlike the normal `clausesOf` path). One printing of Shatterpoint reads "Use the Force **(lose your Force token)**. If you do, …"; the un-stripped parenthetical defeated the `use_force` template, and since *one* unparsed mode voids the whole modal (by design, so a card never fires a subset of its choices), the card produced **zero abilities and silently did nothing**. Fix: `stripReminders` each option in `parseModalEffect`. Both printings now reach full coverage. Regression scenario added (translate 67 → 68). This was almost certainly the playtest symptom — a red herring earlier (a defeated unit sits in `discard` with `damage` reset to 0, so a naive `findCard` check reads "present, dmg 0" and looks like a no-op; checking the *arena* shows it was defeated).
- **Overwhelming Barrage — NOT a bug; unimplemented mechanic (`coverage: none`).** "Give a friendly unit +2/+2 for this phase. Then, it deals damage equal to its power divided as you choose among any number of other units." It plays as a vanilla event (does nothing) because the matcher emits no ability. Building it needs two new pieces: (a) **power-based divided damage** — `divided_damage` currently takes a fixed `amount`, not `amountFromPower`; and (b) a **cross-clause "it" binding** — clause 2's "it"/"its power" is the *same* unit chosen in clause 1, and "other units" = the pool minus that unit. The engine has no let-binding to carry a chosen target between sequenced effects (closest is `trigger_source`). Deferred as a tracked coverage gap, consistent with the coverage-deprioritized stance — not half-built.

### 2026-06-03: Resource-zone play + Plot (§19), session 63g

**The "play a card from the resource zone" architecture, then Plot on top of it.** First a behavior-preserving refactor: pulled the post-cost half of `applyPlayCard` (move the card to its play zone by type; run onPlay hooks / Ambush / event When-Played) into a shared `resolveCardIntoPlay` that doesn't advance the turn. Then `playFromResourceZone` builds on it: pay the cost from *other* ready resources (the played card and its replacement can't pay), swap the played card in the resource zone for the top of the deck (exhausted, §19c), and resolve it. **Plot (§19)** wires into `applyDeployLeader` — after a leader deploys, the controller is offered each Plot card in their resource zone (printed cost), with the eligible set snapshotted before any replacement is drawn (§19d). Scoped to units/events (resource-zone upgrades need a host-target choice). Real Plot unit verified. **Smuggle (§14) deliberately deferred:** it reuses this core but its cost is a bracket (`[N resources <aspects>, <additional cost>]` — resources + aspect penalty + arbitrary additional costs), needing a new action and a cost parser; that's the next chunk, plus Piloting's separate play-as-upgrade path. Engine 138 / translate 67 / validate 16; fuzz 500 clean (the refactor touches every card play and every leader deploy, so the fuzz pass is the real safety check).

### 2026-06-03: Exploit + Hidden keywords, session 63f

**Exploit X (§16).** "While playing this card, you may defeat up to X friendly units; each cuts the cost by 2." The interactive sacrifice is resolved in `applyPlayCard` during cost determination, not in `effectiveCost` (which stays deterministic) — a chooser loop where the "Done" option only appears after the minimum-needed sacrifices, so the default/AI chooser sacrifices exactly the minimum (sane play and fuzzer-safe, since `legal.ts` gates affordability at the best-case reduction). Sacrificed units are bumped to lethal so the trailing settle defeats them, putting their When-Defeated in the same window as the played card's When-Played (§16d). X comes from the "Exploit N" text via the translator. Not modeled: granted Exploit and "for each unit you exploited" payoffs.

**Hidden (§18).** "Can't be attacked the phase it was played/deployed/created," with Sentinel overriding (§18b). This needed a notion of "entered this phase," which the engine didn't track — added a single `phaseStartedAtStep` marker set in `startActionPhase` (the one action-phase entry point; attacks only happen in the action phase, so one spot covers it). The restriction is enforced in **both** `attackIllegalReason` and `legal.ts`'s attack generation — the duplication is deliberate: legal.ts builds attacks inline rather than calling `attackIllegalReason`, so without mirroring the rule there, the fuzzer would pick an attack the reducer then rejects. Modeled as a hook-free marker keyword. Engine 136 / translate 67 / validate 16; fuzz 500 clean.

### 2026-06-03: Bounty implemented + deferred-keyword verification, session 63e

**Bounty (§13).** "Bounty — <effect>" is a When-Defeated ability that's *resolved by an opponent* of the unit's controller (the player who defeated it), and optional to collect (§13e). The AST already carried a `controlled_by?: PlayerRef` flag on triggered abilities and the validator accepted it — but the runtime never read it, so it was dead. Wired it in `collectTriggers`: when an ability is `controlled_by:'opponent'`, the trigger is pushed with the opponent as `sourceController`, so effect resolution, the optional prompt, and §3018 active-player ordering all key off the defeating player, while the predicate still matched against the dying unit. Modeled the When-Defeated half only (the engine has no capture event yet). Matcher gained a `parseBountyClause`; 4 of the 5 cards whose text starts with "Bounty —" are now full coverage. Chose to implement Bounty (and not the other four deferred keywords) because it reuses the trigger framework — the rest need new architecture.

**Verification of the other four deferred keywords (now that the rules are committed).** Read each against the comprehensive rules and recorded exactly why each needs more than a marker keyword: **Exploit X** (§16) is an interactive cost reduction inside the Play-a-Card action (defeat up to X friendly units, −2 cost each); **Smuggle [Y]** (§14) and **Plot** (§19) play a card *from the resource zone* and replace it with the top of the deck (Plot also triggers on leader deploy); **Piloting [Y]** (§17) plays a unit *as an upgrade* on a Vehicle. Smuggle/Piloting/Plot share a "play from a non-hand zone / as an alternate type" mechanic; Exploit is independently buildable. Deferred rather than half-implemented. Engine 130 / translate 67 / validate 16.

### 2026-06-03: Resupply (play-as-resource) + Darth Revan (attacks-and-defeats trigger), session 63d

**`play_as_resource` (#3 Resupply).** "Put this event into play as a resource" now moves the played event into its controller's resource zone (exhausted, §2046) instead of the discard pile — net ramp. Implemented as a self-referential effect that acts on the source event (which the event-play flow has already placed in discard) and relocates it. Matcher + fixture + scenarios; real card verified.

**Attacks-and-defeats trigger (#7 Darth Revan).** "When a friendly unit attacks and defeats a unit: …give an Experience token to that friendly unit." Added a trigger on `event.attack_ended` gated by a new `defender_defeated` trigger-predicate, with the reward aimed at `trigger_source` (which already resolves to the attacker for attack events — so "that friendly unit" needed no new selector). The matcher gained the trigger prefixes (friendly-unit and this-unit variants), a "that (friendly) unit" → trigger_source mapping, and "Exhaust this unit/leader" → exhaust-self; the leader's "You may exhaust this leader. If you do, …" composes cleanly as `optional(if_did(exhaust self, give_experience))` from existing primitives. **Non-obvious bug:** at the moment attack triggers are evaluated, a unit defeated in that combat has already been moved to the discard pile *with its damage reset to 0* — so neither "card is gone" nor "card is at lethal damage" detects the defeat. `defender_defeated` instead checks "no longer in an arena" (with the lethal-damage case kept for timing robustness). This is the kind of state-transition timing detail that only surfaces by running it — the first two attempts (find-fails, then lethal-HP) both failed against the real defeat-to-discard flow before the zone check worked. Fixtures + engine/matcher scenarios; real Darth Revan verified (both un-deployed leaderAbilities and deployed leaderUnitAbilities). This closes the last of the seven playtest-reported issues. Engine 128 / translate 66 / validate 16.

### 2026-06-02: Uptime Check workflow — fix unreachable health target + broken alert

**Root cause was two stacked problems, not one.** The `Uptime Check` GitHub Action failed on every run. (1) The health step curled `https://twinsuns.chanfriendly.duckdns.org/health`, but the public reverse proxy only fronts the Next.js frontend — `/` and `/api/*` resolve, `/health` (a backend-only path) 404s; and at first the public host wasn't reachable at all (Christian adjusted reverse-proxy settings mid-session to expose it). The app itself was up the whole time (frontend `200` over Tailscale `100.127.164.49:4000`). (2) The "Input required and not supplied: from" error was the *alert* step: the repo has no `SMTP_USER`/`SMTP_PASSWORD`/`ALERT_EMAIL` secrets (only `PORTAINER_*`), so `from: ${{ secrets.SMTP_USER }}` was empty. The failed ping + the failed alert compounded into the red run.

**Fix.** Added `frontend/src/app/api/health/route.ts` — a proxy route (mirrors the `/api/aspects` pattern) that hits the backend `/health` with `cache:'no-store'` and returns 503 if the backend is unreachable, so the external check gets a real end-to-end signal (Next.js up AND FastAPI reachable) instead of a frontend-only 200. Pointed the workflow at `/api/health`. Marked the email step `continue-on-error: true` with a comment that it needs the 3 SMTP secrets set to actually deliver — otherwise the failed health step is the red signal and GitHub's native failed-scheduled-run email is the fallback. **The `/api/health` route ships in the frontend image, so the public path stays 404 until the next `./deploy.sh`** — until then the check stays red (or point it at `/api/aspects`, already live, 313 bytes, for an immediate-green no-deploy alternative). tsc clean.

### 2026-06-02: Indirect damage — defender assigns (§8.35), session 63c

**`indirect_damage` effect: the recipient distributes it.** TIE Bomber's "Deal 3 indirect damage to the defending player" was modeled as `damage`→opponent_base — it just dumped 3 on the base, so the defender never got to assign it (Christian's "I couldn't assign the indirect damage"). Replaced with a dedicated `indirect_damage` effect that implements §8.35: the chosen player assigns N unpreventable damage among their base + units, divided as they choose, capped per unit at remaining HP, simultaneously, ignoring Shield tokens without consuming them. The decisive design choice is that the **assigning player is the recipient**, so the chooser's `player` field is the recipient — that's what routes the assignment prompt to the defending side in the UI. Chose a new effect over reusing `divided_damage` (which targets units only and is assigned by the source) because the assigner, the base-inclusion, the per-unit HP cap, and the shield-ignore are all different. The damage primitive already skipped shields for `indirect` without consuming them (§8.35.2a came free); passing `unpreventable` additionally bypasses replacement effects. Default/AI chooser puts it all on the recipient's base (behavior-preserving and a reasonable "protect your units" default). Matcher updated; fixture W8_025; 4 engine + 1 matcher scenario; real TIE Bomber verified end-to-end. Note: only `player:'opponent'` ("the defending player"/"your opponent") is emitted — a free-choice-of-player variant can come later if a card needs it. Engine 125 / translate 63 / validate 16.

### 2026-06-02: "Deals combat damage first" + ability-damage logging (session 63b)

**Combat sequencing for "deals combat damage before the defender" (§1618c / §7.5.6d).** Christian's 2nd playtest round confirmed the session-63 fixes (Tarkin deployed ability + TIE Bomber both work) and surfaced Incinerator Trooper: it deals combat damage first, so if it defeats the defender the defender deals none back — but the engine applied combat simultaneously, so it wrongly took damage. Session 62 had explicitly flagged this rule as "a future re-read point if such a card ships." Modeled as a hook-free marker keyword `attacker_combat_first` (same pattern as Ambush) detected from card text in `translate.parseKeywords`; the sequencing lives in `resolveAttack`. When the attacker deals first: defender takes damage, then deals back ONLY if it survives — and its return power is recomputed *after* taking damage so a Grit defender gets the §7.5.6d bonus (the simultaneous §7.5.6c path still snapshots before, so no bonus there). Chose a marker keyword over a new AST ability type because the capability is read at combat time (not a board-state predicate) and the keyword channel already supports granting/detection; the role is encoded in the name (`attacker_…`) so it correctly does nothing while the holder defends. Only the attacker-side scope shipped (no "while defending" card exists yet). Fixtures W8_023/024, 3 engine + 2 translate scenarios.

**Ability damage now appears in the game log.** `GameLog` reads `state.log`, but only `attack.ts` (combat) appended to it — every ability-driven damage, including TIE Bomber's indirect base damage, landed silently. Added a log line in `interpret.applyDamage`. This is visibility only: the indirect-damage *model* (the defending player assigns N among base + units, §8.35) is still wrong — it dumps N on the base — and remains the open #4. Combat damage is unaffected (it bypasses the `damage` effect).

Engine 121 / translate 62 / validate 16, tsc clean, fuzz 300 + play-cli clean.

### 2026-06-02: Playtest triage + upgrade/leader stat data fix (session 63)

**Root cause of "Resilient did nothing": upgrade & leader stats were never captured from the SWU API.** Christian's real-deck playtest surfaced 7 issues; the one he prioritized (#2) turned out to be a systemic card-DATA gap, not engine code. All 145 upgrades had NULL attack/health because `swu_api_client.py` read `power`/`hp` for every non-leader/base card — but the SWU API puts upgrade modifiers in dedicated `upgradePower`/`upgradeHp` fields (e.g. Resilient = +0/+3). So the translator's correct `attack→powerModifier` mapping always saw 0, and no upgrade modified stats. **Same bug class for leaders:** the Leader branch captured epic_action/deploy_box/cost but dropped `power`/`hp`, so `translate.ts` fell back to a generic 3/6 for *every* leader — wrong (Tarkin's deployed unit is 2/7). Fixed both: added an `Upgrade` branch (`attack←upgradePower`, `health←upgradeHp`) and added `power`/`hp` capture to the `Leader` branch. Rebuilt `swu_cards.db` (repo `databases/` + the live backend's `~/.swu/`); now 149 upgrades + 139 leaders carry real stats (negatives handled, e.g. Nowhere to Hide -2/0). Added a verify-step guard so an all-NULL-upgrades regression is loud on the weekly rebuild. **No frontend change** — `translate.ts` and `modifiers.ts` already mapped/summed the modifiers; the data was the only gap. Corpus 2360→2403 (fresh fetch pulled newer cards). Backups: `/tmp/swu_cards.{repo,swu}.bak`.

**Also: the rest of the playtest triage (the other 6).** #1 (TIE Bomber in ground) was a **stale backend** — the uvicorn process had run since May 19, predating the May-20 arena-serialization commit, so deck payloads carried empty `arenas`; restarted it. #5 (Shatterpoint) and #6 (Tarkin deployed ability) **translate fully on current code** (the playtest predated those primitives / the deploy_box→leaderUnitAbilities wiring) — flagged for re-test, not changed. #3 (Resupply, needs play-as-resource), #4 (TIE Bomber indirect, currently dumps on base instead of defender-assigns), #7 (Darth Revan trigger) are real gaps, deferred to NEXT STEPS. Verdicts were grounded by running the matcher per-card and tracing engine paths, not assumed.

**Lesson:** when a real card "does nothing," check the DB row before the engine — the data layer (API field names) is as much a correctness surface as the AST. tsc clean; engine 118 / translate 60 / validate 16.
