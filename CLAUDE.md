...
**SWU rules notes — verify against the official rules, don't author from memory.** Claude has been wrong about SWU rules from memory multiple times (deploy cost, deploy-ready, take-control permanence, and Twin Suns counters); Christian is the authority and the Comprehensive Rules are public. When a rule seems off, check the source before asserting.

**Rules source (the oracle):** the canonical rules should live in **`docs/rules/`** in this repo (Comprehensive Rules PDF + the Twin Suns format insert). If that directory is missing/empty, the rules aren't committed yet — get the current versions from `https://starwarsunlimited.com/rules` and commit them; do NOT rely on a re-downloaded `/tmp` PDF (a session did that and used a stale **v1.1, 1/31/24** copy that predated the Twin Suns insert and newer keywords). Extract text with `pdftotext <pdf> <txt>` and grep it.

**Twin Suns ≠ 1v1 SWU.** This project is the **Twin Suns** format. Some rules differ from base 1v1 — most notably **counters**: Twin Suns replaces "Take the Initiative" with "Take an Available Counter" and has **three** counters (Initiative, Blast, Plan), not just initiative. When a rule is format-sensitive, prefer the Twin Suns insert over the base Comprehensive Rules, and confirm with Christian.

Confirmed rules the engine implements:
...
- **A deployed leader enters the arena READY and can attack the same round (§3.4.4c).** "When a Leader Unit is deployed, it enters the ground arena ready, even if it was exhausted before." This is the *exception* to §3.4.4b, where non-leader units enter play **exhausted**. `reducer.applyDeployLeader` creates the leader-unit with `exhausted: false`; non-leader unit plays stay `exhausted: true`. The engine originally had this backwards (deployed leaders entered exhausted, citing "§v7") — that was a bug, found in session-52 UAT and verified against the Comprehensive Rules. If you find yourself "fixing" deploy to enter exhausted, stop — leaders are the exception.
