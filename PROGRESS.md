...
**(2026-06-09 pm) ORACLE INVESTIGATION DONE + Smuggle implemented.** Oracle: see **[docs/ORACLE_INVESTIGATION.md](docs/ORACLE_INVESTIGATION.md)** — recommendation is to adapt **forceteki's (Karabast, MIT) per-card test corpus** as golden tests (NOT live differential testing); sequence = fuzzer-invariants + LLM-judge now → port forceteki specs (PoC ~10 cards) → LLM-gen bespoke tail gated by validate + oracle. **NEXT CONCRETE STEP toward the oracle:** hand-port forceteki's specs for ~10 cards we already implement (deck cards + W8 fixtures) to prove the porting pipeline + per-card cost. Smuggle (§14): DONE — new `SMUGGLE` action, flat-N bracket cost (aspect penalty unmodeled, consistent), reuses `playFromResourceZone`; fuzz 500 clean. **Remaining chunky true-primitives:** Piloting [16] (dual-play-type), "When you use the Force" trigger, capture-as-event.

**(2026-06-09) RECALIBRATION — matcher templating has hit diminishing returns; the bespoke tier is the next real leverage.** Corpus **788/2283 (34.5%)**. Worked the highest-count "matcher-reach" buckets and found them **inflated ~9×** by compounds (exhaust/ready [bucket 76 → +8 real]; cost-change [54 → +3 real] — the keyword sits inside bespoke compounds that fail for other reasons). Combined with new-primitive batches yielding tens each, the ~750 fully-residual cards are now dominated by bespoke compounds. **Recommendation: stop grinding templates.** Next leverage is the **bespoke tier**: (1) a CORRECTNESS ORACLE (differential vs a reference SWU engine — investigate Karabast — or golden per-card scenarios), then (2) LLM-generated AST specs gated by `validate.ts` + that oracle. Optionally a few *chunky* true-primitives remain for mechanics breadth (Smuggle [34, needs new action + bracket-cost parser], Piloting [16], "When you use the Force" trigger). **Metric: "% verified correct," not "% translated."**

**(2026-06-08) SCALING TO THE FULL POOL — mechanic-level roadmap.** Corpus is **765/2283 fully playable (33.5%)**. Ran a mechanic-level residual tally (re-bucket every unmatched clause by the primitive it needs): **1,535 residual cards; 783 (51%) entirely bespoke** (→ authored/LLM tier, NOT regex). The rest are reusable buckets. **Build order for the TRUE missing primitives** (each a composable building block, ranked by leverage): Smuggle [34] (resource-zone-play core exists; needs a new action-phase action + bracket-cost parser), force-**triggers** [remaining of 25] (the "while the Force is with you" CONDITION is done; "**When you use the Force:**" TRIGGER — e.g. The Father — needs a FORCE_USED event from `use_force` + a trigger condition), Piloting [16] (dual-play-type path), protection/"can't be defeated/returned" [10] (`Modifier.cant` exists — but the "by enemy card abilities" qualifier needs the defeat/bounce path to know its source is an ability vs combat; deferred as not-clean), Support [8], capture-as-event [part of 33]. **DONE this session:** `controls_named` [10] + `has_initiative` [13] leaves; **`defeat_upgrade` effect [17]**; **Force "while the Force is with you" condition** (matcher-reach over the existing `player_has_force_token` predicate). Net 748 → **777** fully playable. **Separately, MATCHER-REACH buckets** (primitive exists, phrasing doesn't parse — parameterized-template work, low engine risk, high count): exhaust/ready specific unit [76], cost-change conditional/per-X [54], search variants [49], give-shield cond/multi [46], disclose payoffs [31] (runtime now exists — just wire), opponent-discards compounds [21]. **The 783 bespoke tail needs a CORRECTNESS ORACLE before mass-authoring/LLM-gen** (differential testing vs a reference SWU engine — investigate Karabast's license/compatibility — or golden per-card scenarios). Re-run the tally anytime: it was a one-off `_mechanic_tally.ts` (deleted) — rebuild from `coverage_report.ts` + a mechanic-regex bucket map. Metric to optimize is **"% verified correct," not "% translated."**
...
**Engine status: ~93% on the correctness axis** (sessions 61–62 closed counters [all 3 Twin Suns], Ambush, trigger ordering, uniqueness-for-real-decks, a 2000-game fuzzer, and a verified 8-keyword set; 2026-06-07 closed the On-Attack-before-combat ordering, §7.x). Card *coverage* remains ~26% corpus-wide / **94% on the real "Test for Claude" deck** (the separate axis, deliberately deprioritized until the engine is table-trustworthy). Latest verified green (2026-06-09 pm: oracle investigation + Smuggle §14 mechanic): engine **186** / translate **108** / validate 16, tsc clean, fuzz (500) clean. Corpus 788/2283 = 34.5% playable (Smuggle is a keyword, not counted in coverage; the win is the mechanic working in play). Deck "Test for Claude" still **49/52 (94%)**. **First live playtest done** — surfaced one real engine gap (Captain Rex's `When Deployed:` had no matcher prefix → no token; fixed) plus two UI asks (hover-to-preview card text in /playtest; restyle the save/update dialog to the Twin Suns theme) — all three addressed.
