...
**Engine status: ~92% on the correctness axis** (sessions 61–62 closed counters [all 3 Twin Suns], Ambush, trigger ordering, uniqueness-for-real-decks, a 2000-game fuzzer, and a verified 8-keyword set). Card *coverage* remains ~26% corpus-wide / **58% on the real "Test for Claude" deck** (the separate axis, deliberately deprioritized until the engine is table-trustworthy). Latest verified green (2026-06-06 Nimble Prowess whole-card): engine **167** / translate **99** / validate 16, tsc clean, fuzz (400) clean. Deck "Test for Claude" now **45/52 (87%)**.

**Strategy shift → finishing WHOLE cards (Christian, 2026-06-06).** Done: **The Darksaber** (`controller_distinct_keywords` predicate + "ready attached unit") — found a real engine bug (`findCard` didn't search attached upgrades → upgrade-own `if`-conditionals were always-true; fixed). **Nimble Prowess** (`host_arena` zone filter — "a unit in attached unit's arena"). Both upgrade When-Played abilities now work thanks to the findCard fix. Lesson: whole-card work surfaces bugs clause-templating hides. Remaining deck gaps (harder): Chancellor Palpatine (Spy tokens + give-those-tokens-Sentinel — needs a created-token reference), Condemn (while-attacking grant + lose-all-abilities), Corvus (attach a Pilot — Piloting), Death Star Plans (upgrade control-transfer + passive discount), Regional Governor (name a card), Overwhelming Barrage (power-divided — deferred). NOTE: left-play predicate built but unlocked 0 cards; credit tokens PLAY_CARD-only.
