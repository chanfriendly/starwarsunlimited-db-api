...
3. **Engine correctness (~92% → higher), now that rules ARE committed in `docs/rules/`:**
   - **Deferred keywords — VERIFIED against the committed rules (session 63e); Bounty IMPLEMENTED.** **Bounty** (§13) done — opponent-resolved When-Defeated trigger via the now-wired `controlled_by` flag (4/5 direct "Bounty —" cards covered). The other four need new architecture, not just a marker: **Exploit X** (§16) = interactive cost reduction during the Play-a-Card action (defeat up to X friendly units, −2 each) — needs a play-time sacrifice choice in `applyPlayCard`/affordability; **Smuggle [Y]** (§14) and **Plot** (§19) = play a card *from the resource zone* (+ replace it with the top of deck) — needs a new play-source path (Plot also hooks leader-deploy); **Piloting [Y]** (§17) = play a unit *as an upgrade* on a Vehicle — needs a dual-play-type path. Smuggle/Piloting/Plot share a "play from a non-hand zone / as an alternate type" mechanic that's a meaningful build; Exploit is independently doable next.
...
**Engine status: ~92% on the correctness axis** (sessions 61–62 closed counters [all 3 Twin Suns], Ambush, trigger ordering, uniqueness-for-real-decks, a 2000-game fuzzer, and a verified 8-keyword set). Card *coverage* remains ~26% (the separate axis, deliberately deprioritized until the engine is table-trustworthy). Latest verified green (session 63e): engine **130** / translate **67** / validate 16, tsc clean, play-cli + fuzz (300) clean.
