...
1. **Christian: real-deck playtest.** Load one of your actual decks in `/playtest` (DECK SOURCE → MY SAVED DECKS; needs login) and play it out. The fuzzer proves the engine survives random play; this surfaces what random play doesn't — real card interactions, UI/UX, and any text-ability gaps. Report anything that feels wrong; known-inert cards (unmatched text) are expected, not bugs.
2. **Christian: commit current rules into the repo.** The engine's correctness oracle is the SWU Comprehensive Rules, but **no rules file is committed** — this session used a stale cached PDF (Comprehensive Rules **v1.1, 1/31/24**, from `https://files.rebel.pl/files/instrukcje/EN-Comprehensive-Rules_Star-Wars-Unlimited.pdf`, extracted to `/tmp/swu_rules.txt` — a temp file, now gone). It predates the Twin Suns insert (Blast/Plan) and the newer-set keywords. **Action:** drop the current Comprehensive Rules PDF + the Twin Suns format insert into `docs/rules/` (latest at `https://starwarsunlimited.com/rules`). Then the deferred keywords below become verifiable.
3. **Then (engine correctness, ~92% → higher):** verify Bounty/Exploit/Smuggle/Piloting/Plot against the now-committed rules; replacement-effect ordering when multiple match (CR §3200, same chooser pattern as the session-61 trigger-ordering fix); the rare §3018 "opponent-batch-first" counter option.

**Engine status: ~92% on the correctness axis** (sessions 61–62 closed counters [all 3 Twin Suns], Ambush, trigger ordering, uniqueness-for-real-decks, a 2000-game fuzzer, and a verified 8-keyword set). Card *coverage* remains ~26% (the separate axis, deliberately deprioritized until the engine is table-trustworthy). All work sessions 61–62 is verified green: engine **118** / translate **60** / validate 16, tsc clean, play-cli + fuzz clean.
